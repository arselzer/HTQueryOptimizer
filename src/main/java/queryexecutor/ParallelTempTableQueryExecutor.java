...
public class ParallelTempTableQueryExecutor extends TempTableQueryExecutor {
    ...
    private List<Long> stageRuntimes = new LinkedList<Long>();
    ...
    public List<ParallelQueryExecution> listQueryExecutions(String queryStr, boolean enumerateRerootings) throws QueryConversionException, TableNotFoundException, SQLException, IOException {
        sqlQuery = new SQLQuery(queryStr, schema);
        sqlQuery.setDecompositionOptions(decompositionOptions);
        sqlQuery.setCreateIndexes(createIndexes);
        sqlQuery.setApplyAggregates(applyAggregates);
        ...
        sqlQuery.setTablespace(tablespace);
        return sqlQuery.enumerateParallelExecutions(false, enumerateRerootings);
    }
                                    ...
                                    if (analyze) {
                                        ...
                                        if (!query.startsWith("CREATE VIEW")) {
                                            ...
                                            while (rsExplain.next()) {
                                                analyzeJSONString += rsExplain.getString(1);
                                            }
